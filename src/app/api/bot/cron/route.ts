...
/**
 * GET /api/bot/cron — Called by Vercel Cron every 15 min during market hours
...
export async function GET(request: NextRequest) {
  ...
  const logs: string[] = [];
  const log = (msg: string) => { logs.push(msg); console.log(`[cron] ${msg}`); };
  ...
  try {
    ...
    for (const cycle of activeCycles) {
      ...
      for (const contract of openContracts) {
        try {
          ...
          if (profitPct >= 50) {
            log(`${symbol}: 50% profit hit (${profitPct.toFixed(0)}%), closing`);
            ...
            await sql`INSERT INTO "TradeLog" (id, timestamp, level, ticker, message) VALUES (${genId()}, now(), 'TRADE', ${symbol}, ${`CLOSED at 50% profit: ${contract.symbol}`})`;
          }
        } catch { /* skip */ }
      }
    ...
    for (const ticker of tickers) {
      ...
      if (stage === "SELLING_PUTS") {
        ...
        if (account.cash < put.strikePrice * 100) { log(`${symbol}: Not enough cash`); continue; }
        ...
        account.cash -= put.strikePrice * 100;
      }
